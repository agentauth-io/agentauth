...
class RiskService:
    ...
    def _cache_assessment(self, cache_key: str, assessment: RiskAssessment):
        ...
        if len(self._risk_cache) > 10000:
            oldest_keys = sorted(
                self._risk_cache.keys(), key=lambda k: self._risk_cache[k][1]
            )[:5000]
    ...
    async def assess(
        ...
        try:
            ...
            feature_array = [
                feature_dict.get(f, 0.0) for f in anomaly_service.feature_names
            ]
        ...
        if decision == RiskDecision.BLOCK:
            ...
            if fraud_prediction and fraud_prediction.top_risk_factors:
                ...
                recommendations.append(
                    f"Primary risk factor: {top.get('factor', 'unknown')}"
                )
