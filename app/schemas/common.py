...
class ErrorResponse(AgentAuthBaseModel):
    ...
    details: list[ErrorDetail] | None = Field(
        default=None, description="Detailed errors"
    )
    request_id: str | None = Field(default=None, description="Request ID for debugging")

    @classmethod
    def from_exception(
        cls, error: Exception, request_id: str | None = None
    ) -> "ErrorResponse":
        """Create error response from exception."""
        return cls(
            error=error.__class__.__name__, message=str(error), request_id=request_id
        )
...
class ValidationErrorResponse(ErrorResponse):
    ...
    def from_validation_errors(
        cls, errors: list[dict], request_id: str | None = None
