...
class VerifyTransaction(BaseModel):
    ...
    amount: float = Field(..., gt=0, description="Transaction amount being verified")
    currency: str = Field(
...
class VerifyRequest(BaseModel):
    ...
    merchant_id: str | None = Field(
        None, description="Merchant's identifier for additional validation"
    )
            ...
            "examples": [
                {
                    ...
                    "transaction": {"amount": 347.00, "currency": "USD"},
                }
            ]
        }
    }
...
class ConsentProof(BaseModel):
    ...
    consent_id: str = Field(..., description="Original consent identifier")
    ...
    actual_amount: float = Field(..., description="Actual transaction amount")
...
class VerifyResponse(BaseModel):
    ...
    valid: bool = Field(..., description="Whether the authorization is valid")
    authorization_id: str | None = Field(
        None, description="Unique authorization identifier"
    )
    consent_proof: ConsentProof | None = Field(
        None, description="Proof of consent for chargeback defense (if valid)"
    )
    verification_timestamp: datetime = Field(
        ..., description="When verification was performed"
    )
    proof_token: str | None = Field(
        None, description="Signed token containing the proof (store for disputes)"
    )
    error: str | None = Field(None, description="Error reason if not valid")
            ...
            "examples": [
                {
                    ...
                    "proof_token": "eyJ...",  # nosec: B105 - Example JWT token, not a password
                },
                {
                    "valid": False,
                    "verification_timestamp": "2026-01-11T15:02:00Z",
