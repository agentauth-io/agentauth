...
class RiskAssessmentSchema(BaseModel):
    ...
    risk_score: float = Field(
        ..., description="Combined risk score (0.0 - 1.0)", ge=0, le=1, examples=[0.15]
    )
    decision: str = Field(
        ...,
    ...
    assessment_time_ms: float = Field(..., description="Time taken for risk assessment")
    fraud_detection: dict[str, Any] | None = Field(
        None, description="Fraud detection model results"
    )
    anomaly_detection: dict[str, Any] | None = Field(
        None, description="Anomaly detection results"
    )
    factors: dict[str, Any] = Field(
        default_factory=dict, description="Detailed risk factors"
    )
    recommendations: list[str] = Field(
        default_factory=list, description="Risk-based recommendations"
    )
...
class AuthorizeResponse(BaseModel):
    ...
    authorization_code: str | None = Field(
        None, description="One-time code for merchant to verify (only if ALLOW)"
    )
    expires_at: datetime | None = Field(
        None, description="When the authorization code expires"
    )
    consent_id: str | None = Field(
        None, description="Reference to the original consent"
    )
    ...
    step_up_url: str | None = Field(
        None, description="URL for user confirmation (only if STEP_UP)"
    )
    # Risk assessment fields (new)
    risk_assessment: RiskAssessmentSchema | None = Field(
        None, description="ML-based risk assessment results"
    )
