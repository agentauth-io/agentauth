...
class EncryptionKey:
    ...
    def _encrypt_fallback(self, plaintext: bytes, nonce: bytes, aad: bytes) -> bytes:
        ...
        # XOR encrypt
        ciphertext = bytes(p ^ k for p, k in zip(padded, keystream))
    ...
    def _decrypt_fallback(self, ciphertext: bytes, nonce: bytes, aad: bytes) -> bytes:
        ...
        # Derive keystream (same number of iterations as encryption)
        iterations = (len(encrypted) + 31) // 32
        keystream = b''
        for i in range(iterations):
            keystream += hmac.new(
                self._key,
        ...
        # XOR decrypt
        padded = bytes(c ^ k for c, k in zip(encrypted, keystream))
...
class KeyManager:
    ...
    def __init__(self, master: MasterSecret | None = None):
        """
