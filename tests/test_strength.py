    ...
    try:
        resp = requests.post(
            ...
            params={
                ...
                "owner": f"strength-tester-{secrets.token_hex(4)}",
            },
            timeout=10,
        )
...
def make_auth_request(
    api_key: str, payload: dict, timeout: int = 10
) -> tuple[int, dict]:
...
def test_authentication_security():
    ...
    if not api_key:
        results.add(
            "Auth Security", "Get API Key", False, "Could not bootstrap API key"
        )
        return

    results.add(
        "Auth Security", "Bootstrap API Key", True, f"Got key: {api_key[:20]}..."
    )

    # Test: No API key
    status, resp = make_auth_request(
        "", {"agent_id": "test", "action": "read", "resource": "data"}
    )
    results.add(
        "Auth Security", "Reject No API Key", status in [401, 403], f"Status: {status}"
    )

    # Test: Invalid API key format
    status, resp = make_auth_request(
        "invalid_key", {"agent_id": "test", "action": "read", "resource": "data"}
    )
    results.add(
        "Auth Security",
    ...
    status, resp = make_auth_request(
        fake_key, {"agent_id": "test", "action": "read", "resource": "data"}
    )
    results.add(
        "Auth Security", "Reject Random Key", status in [401, 403], f"Status: {status}"
    )

    # Test: Partial valid key
    partial_key = api_key[: len(api_key) // 2]
    status, resp = make_auth_request(
        partial_key, {"agent_id": "test", "action": "read", "resource": "data"}
    )
    results.add(
        "Auth Security", "Reject Partial Key", status in [401, 403], f"Status: {status}"
    )

    # Test: Modified key (bit flip)
    if len(api_key) > 10:
        modified_key = (
            api_key[:10] + ("a" if api_key[10] != "a" else "b") + api_key[11:]
        )
        status, resp = make_auth_request(
            modified_key, {"agent_id": "test", "action": "read", "resource": "data"}
        )
        results.add(
            "Auth Security",
            "Reject Modified Key",
    ...
    status, resp = make_auth_request(
        api_key, {"agent_id": "test", "action": "read", "resource": "data"}
    )
    ...
    sql_key = "' OR '1'='1"
    status, resp = make_auth_request(
        sql_key, {"agent_id": "test", "action": "read", "resource": "data"}
    )
    results.add(
        "Auth Security",
        "Block SQL Injection in Header",
        status in [401, 403],
        f"Status: {status}",
    )
...
def test_rate_limiting():
    ...
    results.add(
        "Rate Limiting",
        f"Handle {request_count} Rapid Requests",
        success_count > 0,
        f"{success_count} succeeded, {rate_limited_count} rate-limited, {rps:.1f} req/s",
    )
    ...
    if rate_limited_count > 0:
        results.add(
            ...
            "Rate Limiter Active",
            True,
            f"Blocked {rate_limited_count} requests",
        )
    else:
        results.add(
            "Rate Limiting",
            "Rate Limiter Active",
            True,
            "No rate limiting (check if intentional)",
            warning=True,
        )
    ...
    status, _ = make_auth_request(
        api_key, {"agent_id": "recovery-test", "action": "read", "resource": "data"}
    )
    results.add(
        "Rate Limiting", "Recovery After Burst", status == 200, f"Status: {status}"
    )
...
def test_input_validation():
    ...
    injection_payloads = [
        ...
        (
            ...
            {
                "agent_id": "test",
                "action": "' UNION SELECT * FROM api_keys --",
                "resource": "data",
            },
        ),
        (
            "SQL Injection - Boolean",
            {"agent_id": "test' AND '1'='1", "action": "read", "resource": "data"},
        ),
        # NoSQL Injection
        (
            "NoSQL Injection",
            {"agent_id": {"$gt": ""}, "action": "read", "resource": "data"},
        ),
        (
            "NoSQL Injection - Where",
            {"agent_id": "test", "action": {"$where": "1==1"}, "resource": "data"},
        ),
        # Command Injection
        (
            "Command Injection - Semicolon",
            {"agent_id": "test; rm -rf /", "action": "read", "resource": "data"},
        ),
        (
            "Command Injection - Backtick",
            {"agent_id": "`cat /etc/passwd`", "action": "read", "resource": "data"},
        ),
        (
            "Command Injection - Pipe",
            {
                "agent_id": "test | cat /etc/passwd",
                "action": "read",
                "resource": "data",
            },
        ),
        ...
        (
            ...
            {"agent_id": "test", "action": "read", "resource": "../../../etc/passwd"},
        ),
        (
        ...
        (
            "XSS - Event Handler",
            {
                "agent_id": "<img onerror=alert('xss') src=x>",
                "action": "read",
                "resource": "data",
            },
        ),
        ...
        (
            "LDAP Injection",
            {"agent_id": "test)(|(password=*))", "action": "read", "resource": "data"},
        ),
        # XML/XXE
        (
            "XXE Attempt",
            {
                "agent_id": "<?xml version='1.0'?><!DOCTYPE foo [<!ENTITY xxe SYSTEM 'file:///etc/passwd'>]>",
                "action": "read",
                "resource": "data",
            },
        ),
        ...
        (
            ...
            {"agent_id": "{{7*7}}", "action": "read", "resource": "data"},
        ),
        (
        ...
        (
            "Unicode Null Byte",
            {"agent_id": "test\x00admin", "action": "read", "resource": "data"},
        ),
        (
            "Unicode Homoglyph",
            {"agent_id": "аdmin", "action": "read", "resource": "data"},
        ),  # Cyrillic 'а'
        # Oversized inputs
        (
            "Oversized Agent ID",
            {"agent_id": "A" * 10000, "action": "read", "resource": "data"},
        ),
        (
            "Oversized Action",
            {"agent_id": "test", "action": "A" * 10000, "resource": "data"},
        ),
        # Special characters
        (
            "Null Characters",
            {"agent_id": "test\x00\x00", "action": "read\x00", "resource": "data"},
        ),
        (
            "Control Characters",
            {"agent_id": "test\x01\x02\x03", "action": "read", "resource": "data"},
        ),
    ]
    ...
    for name, payload in injection_payloads:
        try:
            ...
            leaked_data = any(
                x in resp_str.lower() for x in ["/bin/bash", "/etc/passwd", "password="]
            )
            ...
            results.add(
                "Input Validation",
                name,
                is_safe and not leaked_data,
                f"Status: {status}, Safe: {not leaked_data}",
            )
        except Exception as e:
            results.add(
                "Input Validation", name, True, f"Request failed safely: {str(e)[:50]}"
            )
...
def test_cryptographic_strength():
    ...
    if len(keys) >= 2:
        ...
        results.add(
            ...
            "API Key Length",
            avg_len >= 40,
            f"Average length: {avg_len:.0f} chars",
        )
        ...
        for i, key in enumerate(keys[:3]):
            ...
            results.add(
                ...
                f"Key Entropy #{i+1}",
                len(char_set) >= 10,
                f"{len(char_set)} unique chars in {len(key)} char key",
            )

        # Check for common patterns
        has_pattern = any(
            "1234" in k or "abcd" in k or "0000" in k or k == k[::-1] for k in keys
        )
        results.add(
            "Crypto Strength",
            "No Obvious Patterns",
        ...
        results.add(
            ...
            "Key Generation",
            False,
            "Could not generate multiple keys",
        )
    ...
    if api_key:
        ...
        for _ in range(20):
            ...
            make_auth_request(
                api_key,
                {"agent_id": "timing-test", "action": "read", "resource": "data"},
            )
...
def test_performance():
    ...
    for i in range(50):
        ...
        status, _ = make_auth_request(
            api_key,
            {"agent_id": f"latency-test-{i}", "action": "read", "resource": "data"},
        )
...
def test_edge_cases():
    ...
    edge_cases = [
        ...
        (
            "Whitespace Agent ID",
            {"agent_id": "   ", "action": "read", "resource": "data"},
        ),
        (
            "Newline in Agent ID",
        ...
        (
            "Tab in Resource",
            {"agent_id": "test", "action": "read", "resource": "data\tfile"},
        ),
        # Boundary lengths
        ("1 Char Agent ID", {"agent_id": "a", "action": "r", "resource": "d"}),
        (
            "Max Length Strings",
            {"agent_id": "a" * 255, "action": "b" * 255, "resource": "c" * 255},
        ),
        # Special values
        ...
        (
            "Array Agent ID",
            {"agent_id": ["a", "b"], "action": "read", "resource": "data"},
        ),
        (
            "Nested Object",
            {"agent_id": {"nested": "value"}, "action": "read", "resource": "data"},
        ),
        # Unicode
        (
            "Unicode Emoji",
            {"agent_id": "agent-🤖", "action": "read", "resource": "📁data"},
        ),
        (
            "Unicode RTL",
            {"agent_id": "test\u202eagent", "action": "read", "resource": "data"},
        ),
        (
            "Unicode Zero Width",
            {"agent_id": "te\u200bst", "action": "read", "resource": "data"},
        ),
        # Numbers as strings
        (
            "Numeric String ID",
            {"agent_id": "12345", "action": "read", "resource": "data"},
        ),
        ("Float String", {"agent_id": "123.456", "action": "read", "resource": "data"}),
        (
            "Scientific Notation",
            {"agent_id": "1e10", "action": "read", "resource": "data"},
        ),
        # Special actions
        ...
        (
            "Execute Action",
            {"agent_id": "test", "action": "execute", "resource": "/bin/bash"},
        ),
        # Missing fields
...
def test_token_security():
    ...
    results.add(
        "Token Security",
        "Key Not Echoed in Response",
        not key_leaked,
        (
            "API key not found in response"
            if not key_leaked
            else "⚠️ API key found in response!"
        ),
    )
    ...
    if api_key.startswith("aa_"):
        ...
        results.add(
            ...
            "Proper Key Prefix",
            False,
            f"Unexpected prefix: {api_key[:10]}",
        )
    ...
    status_upper, _ = make_auth_request(
        upper_key, {"agent_id": "test", "action": "read", "resource": "data"}
    )
    status_lower, _ = make_auth_request(
        lower_key, {"agent_id": "test", "action": "read", "resource": "data"}
    )
    status_original, _ = make_auth_request(
        api_key, {"agent_id": "test", "action": "read", "resource": "data"}
    )

    case_sensitive = (
        status_upper != 200 or status_lower != 200
    ) and status_original == 200
    results.add(
        "Token Security",
        "Case Sensitive Keys",
        True,  # Most keys are case-sensitive by design
...
def test_policy_engine():
    ...
    test_scenarios = [
        ...
        (
            "Read Permission",
            {"agent_id": "reader-agent", "action": "read", "resource": "public/data"},
        ),
        (
            "Write Permission",
            {"agent_id": "writer-agent", "action": "write", "resource": "user/data"},
        ),
        (
            "Delete Permission",
            {"agent_id": "admin-agent", "action": "delete", "resource": "temp/file"},
        ),
        (
            "Execute Permission",
            {
        ...
        (
            "Nested Resource",
            {"agent_id": "test", "action": "read", "resource": "a/b/c/d/e/f"},
        ),
...
def test_audit_trail():
    ...
    test_requests = [
        {
            "agent_id": f"audit-test-{unique_id}-1",
            "action": "read",
            "resource": "audit-data",
        },
        {
            "agent_id": f"audit-test-{unique_id}-2",
            "action": "write",
            "resource": "audit-data",
        },
        {
            "agent_id": f"audit-test-{unique_id}-3",
            "action": "delete",
            "resource": "audit-data",
        },
    ]
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if isinstance(transactions, list):
                ...
                results.add(
                    ...
                    "Transactions Recorded",
                    False,
                    f"Unexpected format: {type(transactions)}",
                )
            ...
            results.add(
                ...
                True,
                "Endpoint not implemented (check if intentional)",
                warning=True,
            )
        else:
            results.add(
        ...
        results.add(
            "Audit Trail", "Transactions Endpoint", False, f"Error: {str(e)[:50]}"
        )
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            results.add(
                "Audit Trail",
                "Metrics Tracking",
                has_request_count and has_approval_count,
            ...
            results.add(
                ...
                "Metrics Tracking",
                False,
                f"Metrics status: {resp.status_code}",
            )
    except Exception as e:
...
def test_infrastructure():
    ...
    try:
        ...
        results.add(
            "Infrastructure",
            "CORS Policy",
            not is_open_cors or cors_origin == "",
    ...
    try:
        resp = requests.get(
            f"{BASE_URL}/v1/health", timeout=10
        )  # Use v1 endpoint to get nginx headers
        ...
        results.add(
            "Infrastructure", "Security Headers", False, f"Error: {str(e)[:50]}"
        )
    ...
    try:
        ...
        results.add(
            ...
            (
                ...
                else "⚠️ Sensitive info in errors"
            ),
        )
    except Exception:
        results.add(
    ...
    for method in ["TRACE", "TRACK", "DEBUG"]:
        try:
            ...
            results.add(
                "Infrastructure",
                f"Block {method} Method",
                blocked,
                f"Status: {resp.status_code}",
            )
        except:
            results.add(
                "Infrastructure", f"Block {method} Method", True, "Method rejected"
            )
...
""")
    ...
    for cat, counts in categories.items():
        ...
        print(
            f"  {status_icon} {cat}: {counts['passed']}/{total_cat} ({cat_rate:.0f}%)"
        )
